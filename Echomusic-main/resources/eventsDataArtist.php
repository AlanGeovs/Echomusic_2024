...
switch($typeEvent){
  case '1':
      ...
      $queryEvents = mysqli_query($conn,"SELECT * FROM events_private LEFT JOIN users AS user_sell ON events_private.id_user_sell = user_sell.id_user
                                                                      ...
                                                                      LEFT JOIN name_plan ON events_private.id_name_plan = name_plan.id_name_plan WHERE id_event='$id' AND (id_user_sell='$userid' OR id_user_buy='$userid')");
  ...
  case '3':
      ...
      $queryEvents = mysqli_query($conn,"SELECT * FROM events_private LEFT JOIN users AS user_sell ON events_private.id_user_sell = user_sell.id_user
                                                                      ...
                                                                      LEFT JOIN events_linked ON events_linked.id_event_private = events_private.id_event WHERE events_linked.id_event='$id' AND (events_private.id_user_sell='$userid' OR events_private.id_user_buy='$userid')");
      $events = mysqli_fetch_array($queryEvents);
...
// Display Event Detail Test
  switch($typeEvent){
    // Private events
    case '1':
      if($events['id_user_sell'] == $userid):?>
        <!-- // NEW -->
            ...
            <table id="tableresponsive-event" class="table">
              <tbody>
                <tr class="tr-responsive">
                  ...
                  <td><?=ucfirst($events['first_name_user'])?> <?=ucfirst($events['last_name_user'])?></td>
                </tr>
                <tr class="tr-responsive">
                ...
                <tr class="tr-responsive">
                  ...
                  <td>Nº<?=$events['slot_plan']?> | <?=$events['name_plan']?> | $<?=number_format($events['value_plan_event'] , 0, ',', '.')?></td>
                </tr>
                <tr class="tr-responsive">
                    ...
                    <? switch($events['status_event']):?><?case "reserved": ?>
                        <form action="" method="post">
                            ...
                            <? if($events['date_event']>$timeNow): ?>
                              <button type="submit" class="btn btn-outline-secondary m-2" name="accept_event">Aceptar</button>
                              <button type="submit" class="btn btn-outline-secondary m-2" name="reject_event">Rechazar</button>
                            <? elseif($events['date_event']<$timeNow): ?>
                              <a class="btn btn-outline-secondary m-2 isDisabled">Aceptar</a>
                              <a class="btn btn-outline-secondary m-2 isDisabled">Rechazar</a>
                            <? endif; ?>
    ...
    <? elseif($events['id_user_buy'] == $userid): ?>
    ...
    <? case '3': ?>
      <? if($events['id_user_sell'] == $userid): ?>
        ...
        <ul class="alt">
          <li id="event_date"><strong>Cliente: </strong>'.ucfirst($events['first_name_user']).' '.ucfirst($events['last_name_user']).'</li>
          <li id="event_date"><strong>Dirección: </strong>'.ucfirst($events['location']).'</li>
          <li id="event_date"><strong>Comuna: </strong>'.ucfirst($events['name_city']).'</li>
          <li id="event_date"><strong>Fecha: </strong>';echo getDayday($date).' de '; echo getMonthYear($date); echo '</li>
          <li id="event_date"><strong>Hora: </strong>'.$time.'</li>
          <li id="event_date"><strong>Plan: </strong>Nº'.$events['slot_plan'].' <strong>Tipo de Plan:</strong> '.$events['name_plan'].' <strong>Valor:</strong> $'.number_format($events['value_plan_event'] , 0, ',', '.').'</li>
          <li id="event_status"><strong>Estado: </strong>'.$statusEvent.'</li>
          <li id="event_desc"><strong>Descripción: </strong>'.$events['desc_event'].'</li>
        </ul>';
          echo '<form action="" method="post">';
            echo '<ul class="actions">';
              echo '<input type="hidden" value="'.$events['id_event'].'" name="id_event" />';
              echo '<li><a onClick="writeCancelValue('.$events['id_event'].')" href="#cancel_event" class="button">Cancelar Evento</a> </li>';
              echo '<li><a onClick="showList()" class="button">volver</a></li>';
            echo '</ul>';
          echo '</form>'; -->
      <? elseif($events['id_user_buy'] == $userid): ?>
        <!-- echo
