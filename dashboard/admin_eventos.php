    ...
    <body class="light sidebar-mini sidebar-collapse">
                                ...
                                <div class="row justify-content-between mb-0 mt-15">
                                    <div class="col-md-4">
                                        <div class="card-header white b-0 p-3">
                                            <h4 class="card-title">Administración de Eventos</h4>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-8 d-flex align-items-center justify-content-center">
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-md-3 mb-0 ">
                                                <button type="button" class="btn btn-primary mt-2" onclick="buscarEventos()">
                                ...
                                <div class="collapse show" id="invoiceCard">
                                    <div class="card-body p-0">
                                        <div class="table-responsive">
                                            <table id="recent-events" class="table table-hover mb-0 ps-container ps-theme-default">
                                                <thead class="bg-light">
                                                    <tr>
                                                        <th> </th>
                                                        <th>ID Evento</th>
                                                        <th>Nombre de Evento / Artista</th>
                                                        <th>Ciudad / Región</th>
                                                        <th>Organizador / Fecha / Lugar </th>
                                                        <th>Estatus</th>
                                                        <!-- <th>Verificado</th> -->
                                                        <th>Acciones</th>
                                                        <th>Activar</th>
        ...
        <script>
            function cargarUsuarios(pagina = 1, filtro = '') {
                const url = `includes/obtener_eventos_admin.php?pagina=${pagina}&filtro=${encodeURIComponent(filtro)}`;
                fetch(url)
                    .then(response => response.json())
                    .then(respuesta => { // Cambia `eventos` a `respuesta` para evitar confusiones
                        let tabla = document.getElementById('recent-events').getElementsByTagName('tbody')[0];
                        ...
                        respuesta.eventos.forEach(evento => { // Accede a `respuesta.eventos` en lugar de `eventos` directamente
                            ...
                            fila.innerHTML = `
                    <td>
                        <img src="/dashboard/images/eventos/${evento.img}.jpg" width="50px" alt="Evento ${evento.name_event}" style="cursor:pointer;" data-toggle="modal" data-target="#modalImagenEvento" data-img="/dashboard/images/eventos/${evento.img}.jpg"/>
                    </td>
                    <td>${evento.id_event}</td>  
                    <td>
                        <a href="https://echomusic.net/eventos.php?e=${evento.id_event}" target="_blank">${evento.name_event}</a>                                              
                        <br><span class="icon-user"></span> ${evento.datos_artista.first_name_user} ${evento.datos_artista.last_name_user} ${evento.datos_artista.nick_user ? `(<a href="https://echomusic.net/artistas.php?a=${evento.id_user}" target="_blank">${evento.datos_artista.nick_user}</a>)` : ''}
                    </td>
                    <td>
                        ${evento.nombre_ciudadyregion ? evento.nombre_ciudadyregion : ' - '}  
                    </td> 
                    <td>
                           <b>${evento.organizer}</b>
                        <br><span class="icon-calendar-check-o"></span> ${evento.date_event}
                        <br><span class="icon-building2"></span> ${evento.name_location}
                        <br><span class="icon-map-marker"></span> ${evento.location}
                    </td>
                    <td>${obtenerStatus(evento.active_event)}</td>
                    <!-- <td>${validarUsuarioVerificado(evento.verified)}</td>   -->
                    <td>  
                        <!-- <a class="btn-fab btn-fab-sm btn-primary shadow text-white" ><i class="icon-pencil"></i></a> -->
                        <!-- Botón para abrir modal -->
                        <a class="btn-fab btn-fab-sm btn-primary shadow text-white"  href="https://echomusic.net/dashboard/editar_evento.php?id_evento=${evento.id_event}&edit=admin" target="_blank">
                            <i class="icon-pencil"></i>
                        <a class="btn-fab btn-fab-sm btn-success shadow text-white" href="https://echomusic.net/dashboard/ver_entradas.php?id_evento=${evento.id_event}" target="_blank">
                            <i class="icon-ticket"></i>
                        </a>
                        <a class="btn-fab btn-fab-sm btn-danger shadow text-white" onclick="confirmarBorrado(${evento.id_event})">
            ...
            function obtenerStatus(status) {
                switch (status) {
                    case 0:
                        return '<span class="badge badge-light">Desactivado</span>';
                    case 1:
                        return '<span class="badge badge-success">Activo</span>';
                    case 2:
                        return '<span class="badge badge-warning">Pasado</span>';
                    default:
                        ...
                        if (data.success) {
                            swal("Éxito", "Los datos del usuario han sido actualizados correctamente.", "success")
                                .then(() => {
                                    ...
                                    cargarUsuarios(); // Recarga la lista de usuarios si tienes una función para esto
                                });
                        } else {
