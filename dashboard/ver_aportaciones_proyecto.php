...
$idEvento = $_GET['id_project'];
$datosEvento = Consultas::obtenerProyectoPorId($idEvento); //  devuelve los datos del evento
$regionProyecto =  Consultas::obtenerNombresRegion($datosEvento['project_region']);
...
?>
    ...
    <head>
        ...
        <title>Administador de Proyectos</title>
        <!-- CSS -->
    ...
    <body class="light sidebar-mini sidebar-collapse">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <ul class="nav nav-material responsive-tab">
                            ...
                            <li>
                                <a class="nav-link active" href="ver_aportaciones_proyecto.php">
                                    <i class=" icon icon-money"></i>Crowdfunding > Aportaciones
                                </a>
                            </li>
                                ...
                                <div class="row justify-content-between mb-0 mt-15">
                                    <!-- <div class="col-md-2">
                                        <div class="card-header white b-0 p-3">
                                            <img src="/dashboard/images/proyectos/<?php echo htmlspecialchars($datosEvento['img']); ?>.jpg" width="100px" alt="Evento <?php echo htmlspecialchars($datosEvento['project_title']); ?>">
                                        </div>
                                    </div> -->
                                    <div class="col-md-4">
                                        <div class="card-header white b-0 p-3">
                                            <h4 class="card-title">Proyecto: <?php echo htmlspecialchars($datosEvento['project_title']) . " <small> " . htmlspecialchars($datosEvento['id_project']); ?></small></h4>
                                            <p>Fecha de ejecución: <?php echo htmlspecialchars($datosEvento['project_date_execution']); ?>
                                                <?php
                                                if (!$regionProyecto == 0) {
                                                ?>
                                                    <br>Región: <?php echo htmlspecialchars($regionProyecto);
                                                            }
                                                                ?>
                                            </p>
                                        </div>
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-9 mb-0">
                                                <div class="form-group">
                                                    <input class="form-control form-control-lg" type="text" placeholder="Buscar aportaciones" id="filtro">
                                                </div>

                                            </div>
                                            <div class="col-md-3 mb-0 ">
                                                <button type="button" class="btn btn-primary mt-2" onclick="buscarAportaciones()">
        ...
        <script>
            function cargarAportaciones(pagina = 1, filtro = '') {
                var idProyecto = <?php echo json_encode($idEvento); ?>;
                const url = `includes/obtener_aportaciones_admin.php?proyecto=${idProyecto}&pagina=${pagina}&filtro=${encodeURIComponent(filtro)}`;
                fetch(url)
                    ...
                    .then(respuesta => { // Cambia `eventos` a `respuesta` para evitar confusiones
                        ...
                        respuesta.aportaciones.forEach(aportacion => { // Accede a `respuesta.eventos` en lugar de `eventos` directamente
                            let fila = tabla.insertRow();
                            fila.innerHTML = `
                    <td>
                        ${aportacion.id_project_backer}
                    </td>
                    <td>${aportacion.id_tier}</td>  
                    <td>                
                        ${aportacion.datos_artista.first_name_user} ${aportacion.datos_artista.last_name_user} 
                        <br><span class="icon-mail-envelope-closed2"></span> ${aportacion.datos_artista.mail_user}                                                                                        
                        <br><span class="icon-user"></span> ID ${aportacion.id_user}                                                                                        
                    </td> 
                    <td>
                           <b> <span class="icon-ticket"></span> ${aportacion.order_transaction}</b> 
                           <br><span class="icon-date_range"></span> ${aportacion.backer_date}</b> 
                    </td>
                    <td> 
                           <br><span class="icon-date_range"></span>Aportación: $ ${aportacion.backer_amount}</b> 
                           <br><span class="icon-date_range"></span>Extra: $ ${aportacion.backer_added_amount}</b> 
                           <br><span class="icon-date_range"></span>Total: $ ${aportacion.backer_added_amount + aportacion.backer_amount }</b> 
                    </td>
                    <td>   ${obtenerStatus(aportacion.status_backer)}</td> 
                    <td>                          
                        <a class="btn-fab btn-fab-sm btn-success shadow text-white" onclick="reenviarEmail(${aportacion.id_project_backer})">
                            <i class="icon-send"></i>
                        </a>
                        <a class="btn-fab btn-fab-sm btn-danger shadow text-white" onclick="confirmarBorrado(${aportacion.id_project_backer})">
                            <i class="icon-trash"></i></a>
                    </td>
                    <td>
                        <div class="material-switch">
                            <input id="sw${aportacion.id_project_backer}" name="someSwitchOption001${aportacion.id_project_backer}" type="checkbox" ${aportacion.status_backer == 1 ? 'checked' : ''} onchange="cambiarEstadoAportacion(${aportacion.id_project_backer}, this.checked)"> 
                            <label for="sw${aportacion.id_project_backer}" class="bg-primary"></label>
                        </div>
                    </td> 
                `;
                        });
            ...
            //     if (paginaActual > 1) {
            //         paginador.innerHTML += `<li class="page-item"><a class="page-link" href="#" onclick="cargarAportaciones(${paginaActual - 1}, '${filtro}')">Anterior</a></li>`;
            //     }

            //     // Muestra hasta 3 páginas antes de la actual si es posible
            //     for (let i = Math.max(1, paginaActual - 3); i < paginaActual; i++) {
            //         paginador.innerHTML += `<li class="page-item"><a class="page-link" href="#" onclick="cargarAportaciones(${i}, '${filtro}')">${i}</a></li>`;
            //     }
            ...
            //         }
            //         paginador.innerHTML += `<li class="page-item"><a class="page-link" href="#" onclick="cargarAportaciones(${totalPaginas}, '${filtro}')">${totalPaginas}</a></li>`;
            //     }

            //     // Agrega "Siguiente" si no es la última página
            //     if (paginaActual < totalPaginas) {
            //         paginador.innerHTML += `<li class="page-item"><a class="page-link" href="#" onclick="cargarAportaciones(${paginaActual + 1}, '${filtro}')">Siguiente</a></li>`;
            //     }
            // }
            function actualizarPaginador(paginaActual, totalPaginas, filtro = '') {
                let paginador = document.querySelector('.pagination');
                ...
                paginador.innerHTML += `<li class="page-item ${paginaActual === 1 ? 'active' : ''}"><a class="page-link" href="#" onclick="cargarAportaciones(1, '${filtro}')">1</a></li>`;
                ...
                for (let i = Math.max(2, paginaActual - 3); i <= Math.min(paginaActual + 3, totalPaginas - 1); i++) {
                    paginador.innerHTML += `<li class="page-item ${i === paginaActual ? 'active' : ''}"><a class="page-link" href="#" onclick="cargarAportaciones(${i}, '${filtro}')">${i}</a></li>`;
                }
                ...
                if (totalPaginas > 1) {
                    paginador.innerHTML += `<li class="page-item ${paginaActual === totalPaginas ? 'active' : ''}"><a class="page-link" href="#" onclick="cargarAportaciones(${totalPaginas}, '${filtro}')">${totalPaginas}</a></li>`;
                }
            ...
            function confirmarBorrado(idAporte) {
                swal({
                        title: "¿Estás seguro?",
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            fetch(`includes/borrar_aportacion.php?id=${idAporte}`, {
                                    method: 'DELETE',
                                })
                                .then(response => response.json())
                                .then(data => {
            ...
            function cambiarEstadoAportacion(idEvento, estado) {
                fetch('includes/cambiar_estado_aportacion.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({
                            id_project_backer: idEvento,
                            status_backer: estado ? 1 : 0
                        })
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            swal("Éxito", "El estado de la aportación ha sido actualizado.", "success");
                            // Recargar la tabla de eventos
                            const filtro = document.getElementById('filtro').value;
                            cargarAportaciones(1, filtro);
                        } else {
                            swal("Error", "No se pudo actualizar el estado de la aportación.", "error");
                        }
                    })
                    .catch(error => {
            ...
            function buscarAportaciones() {
                const filtro = document.getElementById('filtro').value;
                cargarAportaciones(1, filtro); // Llama a cargarAportaciones con el valor del filtro
            }
            ...
            function reenviarEmail(idAportacion) {
                swal({
                        title: "¿Estás seguro de querer reenviar el correo?",
                        text: "Esto enviará los detalles de la aportación al correo del usuario.",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    })
                    .then((willSend) => {
                        if (willSend) {
                            fetch('includes/reenviar_email_aportacion.php', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
