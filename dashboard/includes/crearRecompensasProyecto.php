...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $idProyectoActual = $_POST['id_project'] ?? null; // Asegúrate que este sea el ID del proyecto
    ...
    for ($i = 1; $i <= 5; $i++) {
        $tierTitle = $_POST["r{$i}_tier_title"] ?? '';
        $tierAmount = $_POST["r{$i}_tier_amount"] ?? '';
        $tierDesc = $_POST["r{$i}_project_desc"] ?? '';
        $rewards = isset($_POST["r{$i}_t_reward"]) ? explode(",", $_POST["r{$i}_t_reward"]) : [];
